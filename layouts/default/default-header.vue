    ...
    <div class="container">
      <div class="content w-100">
        ...
        <ul class="navbar-nav" :class="showNav ? 'active-nav' : ''">
          <template v-for="(nav, i) in navLinks" :key="nav.key">
            <li v-if="!computedMenusArray || (computedMenusArray && computedMenusArray.includes(nav.key))"
              class="nav-item">
...
const computedMenusArray = computed<string[] | undefined>(() => {
  const menus = setting('menus')?.value;
  return menus ? JSON.parse(menus) : undefined;
})
